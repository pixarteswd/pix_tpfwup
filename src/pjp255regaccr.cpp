...
pjp255RegAccr::pjp255RegAccr(HidDevice* dev) :
        mHidDevice(dev), mMaxBurstLength(0)
{
}

pjp255RegAccr::~pjp255RegAccr()
{
}

void pjp255RegAccr::prepare()
{
    /**
...
byte pjp255RegAccr::readuserRegister(byte bank, byte address)
{
    byte buffer[4];
    int len = 0;
...
byte pjp255RegAccr::readRegister(byte bank, byte address)
{
    byte buffer[4];
    int len = 0;
...
int pjp255RegAccr::readRegisters(byte* data, byte bank, byte startAddress,
        uint64_t length)
{
...
int pjp255RegAccr::readUserRegisters(byte* data, byte bank, byte startAddress,
        uint64_t length)
{
    for (uint64_t i = 0; i < length; ++i)
    {
...
int pjp255RegAccr::burstReadRegister(byte* data, byte bank, byte address,
        uint64_t length)
{
...
void pjp255RegAccr::writeRegister(byte bank, byte address, byte value)
{
    byte buffer[] =
    { address, bank, value };
    mHidDevice->setFeature(RW_REG_REPORT_ID, buffer, 3);
}
void pjp255RegAccr::writeUserRegister(byte bank, byte address, byte value)
{
    byte buffer[] =
    { address, bank, value };
    mHidDevice->setFeature(RW_USER_REG_REPORT_ID, buffer, 3);
}
void pjp255RegAccr::writeRegisters(byte bank, byte startAddress, byte* values,
        uint64_t length)
{
    for (uint64_t i = 0; i < length; ++i)
    {
...
byte pjp255RegAccr::readInputReport(void)
{
    bool res = mHidDevice->Read();
    return res;
}
void pjp255RegAccr::burstWriteRegister(byte bank, byte address, byte* values,
        uint64_t length)
{
#ifdef DEBUG
